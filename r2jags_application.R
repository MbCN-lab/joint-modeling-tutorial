...
for (i in 1:n.trials) {
x[i]<-(beta[2*i]-beta[2*i-1])
t[i] ~ dwiener(alpha,tau,omega,x[i])
}
}
"
...
double.gamma.wiener.out = coda.samples(model = model.dgw, 
                   variable.names = c("beta0", "beta", "sigma", "alpha", "tau", "Npred"), 
                   n.iter = 6000) 
...
# Calculate the difference in beta estimates 

xi.mean<-apply(post[,123:162,], 2, function(x) mean(x))
xi.len<-length(xi.mean)/2
xi.diff<-rep(NA,xi.len)
for(i in 1:length(xi.diff)){
  xi.diff[i]<-xi.mean[2*i]-xi.mean[2*i-1]
}
xi.diff
...
xi.diff<- xi.diff[-c(5,6)] 

# Plot 
pdf("delta_rt.pdf",7,6)
lwd=3
cex.big=2.5
cex.tiny=1.5
col=rgb(0,0,1,.25)
...
par(mar=c(5,6,2,2))
plot(comp.rt,xi.diff,xlab="Response Time", pch=c(16,3)[comp.resp], 
     ylab= expression(xi[i]), cex.lab=cex.big)
points(comp.rt[comp.acc==0],xi.diff[comp.acc==0],pch=1,cex=3) #places a circle around the errors
abline(h=0,lty=2)
